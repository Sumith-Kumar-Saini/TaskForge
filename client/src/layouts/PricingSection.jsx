        ...
        <section className="bg-white py-12 px-4">
            ...
            <div className="flex flex-col md:flex-row justify-center gap-8 max-w-4xl mx-auto">
                {plans.map((plan, index) => (
                    <div
                        key={index}
                        className={`flex-1 border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer flex flex-col justify-between ${plan.isPopular ? 'border-black' : 'border-gray-200'
                            }`}
                    >
                        <div>
                            {plan.isPopular && (
